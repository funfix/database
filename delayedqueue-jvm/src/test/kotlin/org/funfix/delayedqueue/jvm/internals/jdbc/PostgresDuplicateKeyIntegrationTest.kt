...
class PostgresDuplicateKeyIntegrationTest {
    ...
    fun startContainer() {
        assumeTrue(
            ...
            "Docker is not available; skipping PostgreSQL tests",
        )
        container =
    ...
    @Test
    fun `duplicateKey should match primary key violation from PostgreSQL`() {
        val ex =
            dataSource.connection.use { conn ->
                conn.createStatement().use { stmt ->
                    stmt.execute("DROP TABLE IF EXISTS test")
                    stmt.execute("CREATE TABLE test (id INT PRIMARY KEY, name TEXT)")
        ...
        assertNotNull(ex)
        assertTrue(PostgreSQLFilters.duplicateKey.matches(ex!!))
    }

    @Test
    fun `duplicateKey should match unique key violation from PostgreSQL`() {
        val ex =
            dataSource.connection.use { conn ->
                conn.createStatement().use { stmt ->
                    stmt.execute("DROP TABLE IF EXISTS test")
                    stmt.execute("CREATE TABLE test (id INT PRIMARY KEY, email TEXT UNIQUE)")
    ...
    fun `duplicateKey should not match non unique constraint violation`() {
        ...
        assertNotNull(ex)
        assertFalse(PostgreSQLFilters.duplicateKey.matches(ex!!))
    }
}
