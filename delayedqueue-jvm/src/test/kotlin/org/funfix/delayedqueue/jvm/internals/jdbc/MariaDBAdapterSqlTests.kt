...
class MariaDBAdapterSqlTests {
    private val tableName = "dq_table"

    @Test
    fun `insert uses INSERT IGNORE pattern`() {
        ...
        assertTrue(sql.contains("INSERT IGNORE INTO $tableName"))
    }
    ...
    @Test
    fun `selectFirstAvailableWithLock uses FOR UPDATE SKIP LOCKED and LIMIT 1`() {
        val capture = SqlCapture()
        val connection = capture.connection()
        val adapter = SQLVendorAdapter.create(JdbcDriver.MariaDB, tableName)
