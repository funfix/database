...
private constructor(
            ...
            database.withTransaction { connection ->
                ...
                if (existing != null) {
                    ...
                    if (existing.data.isDuplicate(newRow)) {
                        ...
                        val updated = adapter.guardedUpdate(connection.underlying, existing.data, newRow)
                        if (updated) {
        ...
        sneakyRaises {
            ...
            toUpdate.forEach { msg ->
                if (msg.message.canUpdate) {
                    val outcome = offer(msg.message.key, msg.message.payload, msg.message.scheduleAt, canUpdate = true)
    ...
    override fun tryPoll(): AckEnvelope<A>? =
        sneakyRaises {
            database.withTransaction { connection ->
                ...
                val row = adapter.selectFirstAvailableWithLock(connection.underlying, pKind, now)
                ...
                val deliveryType =
                    if (row.data.createdAt == row.data.scheduledAtInitially) {
    ...
    override fun tryPollMany(batchMaxSize: Int): AckEnvelope<List<A>> =
        sneakyRaises {
            database.withTransaction { connection ->
                ...
                val rows = adapter.selectAllAvailableWithLock(connection.underlying, lockUuid, count, null)
    ...
    override fun read(key: String): AckEnvelope<A>? =
        sneakyRaises {
            database.withConnection { connection ->
                val row = adapter.selectByKey(connection.underlying, pKind, key)
    ...
    private val cronService: CronService<A> by lazy {
        org.funfix.delayedqueue.jvm.internals.CronServiceImpl(
            ...
            deleteCurrentCron = { configHash, keyPrefix ->
                sneakyRaises {
                    database.withTransaction { connection ->
                        adapter.deleteCurrentCron(connection.underlying, pKind, keyPrefix, configHash.value)
                    }
                }
            },
                ...
                database.withConnection { connection ->
                    val migrations =
                        when (connectionConfig.driver) {
                            ...
                            JdbcDriver.Sqlite,
                            -> throw UnsupportedOperationException("Database ${connectionConfig.driver} not yet supported")
                        }
